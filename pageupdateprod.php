...
if(isset($_FILES['imagem_produto']) && $_FILES['imagem_produto']['error'] === UPLOAD_ERR_OK) {
   ...
   $uploadDirectory = 'images/produtos/';
  ...
  // Move o arquivo de imagem para o diretório de upload
if(move_uploaded_file($_FILES['imagem_produto']['tmp_name'], $uploadFilePath)) {
   // Atualiza o nome da imagem no banco de dados
   $manager->updateImage("loja", $id, $imageName);
...
$operacao = "insert"; //Define a operação padrão como inserção
$id = 0;

// Verifica se há um ID na URL para determinar se é uma operação de atualização
if (isset($_GET["id"])) {
    $operacao = "update";
    $id = $_GET["id"];
    $con = ligaBD();
    $stm = $con->prepare("SELECT * FROM loja WHERE id = ?");
    $stm->bind_param("i", $id);
    $stm->execute();
    $res = $stm->get_result();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($validacao) === 0) {
        ...
        if (isset($_FILES["imagem_produto"])) {
            $targetDir = "images/produtos/";
            $targetFile = $targetDir . basename($_FILES["imagem_produto"]["name"]);
            move_uploaded_file($_FILES["imagem_produto"]["tmp_name"], $targetFile);
        } else {
            $targetFile = ''; // Define um valor padrão se a imagem não for fornecida
        }

        // Verificar se o nome do produto não é nulo
        if (!empty($_POST["nome_produto"])) {
            // Prepara a declaração SQL com base na operação
            if ($operacao == "insert") {
                $stm = $con->prepare("INSERT INTO loja (nome_produto, descricao, preco, tamanho, cor, imagem_produto) VALUES (?,?,?,?,?,?)");
                $stm->bind_param("ssssss", $_POST["nome_produto"], $_POST["descricao"], $_POST["preco"], $_POST["tamanho"], $_POST["cor"], $targetFile);
            } elseif ($operacao == "update") {
                $stm = $con->prepare("UPDATE loja SET nome_produto =?, descricao =?, preco =?, tamanho =?, cor =?, imagem_produto =? WHERE id =?");
                $stm->bind_param('ssssssi', $_POST["nome_produto"], $_POST["descricao"], $_POST["preco"], $_POST["tamanho"], $_POST["cor"], $targetFile, $id);
            }
            ...
            if ($stm->execute()) {
                ...
                echo "Ocorreu um erro ao salvar o registro.";
            }

            $stm->close();
            $con->close();
        } else {
            echo "O nome do produto não pode ser nulo.";
        }
    }
}
?>
...
<?php endif; ?>
  ...
  <div class="content-wrapper"style="background: linear-gradient(to bottom,#00008B, #4169E1);">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0" style="color: #FFFFFF;">Atualizar Produto</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Início</a></li>
              <li class="breadcrumb-item active"style="color: #FFFFFF;">Atualizar Produto</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
  ...
  <div class="form-group mb-3">
    ...
    <input name="imagem_produto" type="file" class="form-control">
    <?php if ($_POST && in_array("imagem_produto", $validacao)) {
      echo "<span class=\"text-danger\">(Preenchimento obrigatório)</span>";
    }?>
  </div>

  <div class="form-group mb-3">
    <label for="nome_produto" class="text-white">Nome do Produto:</label>
    <input name="nome_produto" type="text" class="form-control" value="<?php if (isset($resultados)) echo $resultados["nome_produto"]; elseif ($_POST) echo $_POST["nome_produto"];?>">
    <?php if ($_POST && in_array("nome_produto", $validacao)) {
      echo "<span class=\"text-danger\">(Preenchimento obrigatório)</span>";
    }?>
  </div>
  ...
  <div class="form-group mb-3">
    ...
    <input name="descricao" type="text" class="form-control" value="<?php if ($_POST) echo $_POST["descricao"]; elseif ($operacao == "update") echo $resultados["descricao"];?>">
    <?php if ($_POST && in_array("descricao", $validacao)) {
      echo "<span class=\"text-danger\">(Preenchimento obrigatório)</span>";
    }?>
  </div>
  ...
  <div class="form-group mb-3">
    ...
    <input name="preco" type="text" class="form-control" value="<?php if ($_POST) echo $_POST["preco"]; elseif ($operacao == "update") echo $resultados["preco"];?>">
    <?php if ($_POST && in_array("preco", $validacao)) {
      echo "<span class=\"text-danger\">(Preenchimento obrigatório)</span>";
    }?>
  </div>

  <div class="form-group mb-3">
    <label for="tamanho" class="text-white">Tamanho:</label>
    <input name="tamanho" type="text" class="form-control" value="<?php if ($_POST) echo $_POST["tamanho"]; elseif ($operacao == "update") echo $resultados["tamanho"];?>">
    <?php if ($_POST && in_array("tamanho", $validacao)) {
      echo "<span class=\"text-danger\">(Preenchimento obrigatório)</span>";
    }?>
  </div>

  <div class="form-group mb-3">
    <label for="cor" class="text-white">Cor:</label>
    <input name="cor" type="text" class="form-control" value="<?php if ($_POST) echo $_POST["cor"]; elseif ($operacao == "update") echo $resultados["cor"];?>">
    <?php if ($_POST && in_array("cor", $validacao)) {
      echo "<span class=\"text-danger\">(Preenchimento obrigatório)</span>";
    }?>
  </div>

  <div class="form-group mb-3 text-center">
  <input name="Adicionar" type="submit" value="Alterar" class="btn btn-primary">
</div>
